...
cc_choice_log () {
	...
	local choice="$2" 
	local reasoning="$3" 
	python3 "$PROMPT_LOGGER" choice --prompt-id "$prompt_id" --choice "$choice" --reasoning "$reasoning" 2> /dev/null
}
cc_log () {
...
cc_prompt_log () {
	...
	shift 2
	local options=("$@") 
	python3 "$PROMPT_LOGGER" prompt --type "$prompt_type" --text "$prompt_text" --options "${options[@]}" 2> /dev/null
}
cc_summary () {
...
confirm_with_logging () {
	...
	python3 "$PROMPT_LOGGER" choice --prompt-id "$prompt_id" --choice "$choice" 2> /dev/null
...
prompt_backtrack () {
	local search="${1}" 
	local last="${2:-20}" 
	python3 "$PROMPT_LOGGER" backtrack --search "$search" --last "$last"
}
prompt_replay () {
	local session="${1}" 
...
alias -- @s='adm status'
alias -- @sum='adm summary'
alias -- cc_log='python3 $DB_LOGGER_PATH log'
alias -- ccl=cc_log
alias -- ccmon=cc_monitor
alias -- ccsum=cc_summary
alias -- db_insights='python3 -c "from db_logger_integration import get_insights; get_insights()"'
alias -- db_operations='echo "SELECT COUNT(*) as total, tool_used FROM operations GROUP BY tool_used;" | sqlite3 $UNIFIED_DB_PATH'
alias -- db_predict='python3 -c "from db_logger_integration import predict_next; predict_next()"'
...
export PATH=/Users/MAC/bin\:/Users/MAC/.claude/bin\:/opt/homebrew/bin\:/opt/homebrew/sbin\:/usr/local/bin\:/System/Cryptexes/App/usr/bin\:/usr/bin\:/bin\:/usr/sbin\:/sbin\:/var/run/com.apple.security.cryptexd/codex.system/bootstrap/usr/local/bin\:/var/run/com.apple.security.cryptexd/codex.system/bootstrap/usr/bin\:/var/run/com.apple.security.cryptexd/codex.system/bootstrap/usr/appleinternal/bin
